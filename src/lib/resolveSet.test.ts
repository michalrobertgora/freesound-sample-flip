...
/** Transport that always returns 401 — exercises the auth-failure cascade. */
const unauthorized: Transport = async () => ({ status: 401, json: async () => ({}) });
...
describe("resolveSeededSet request construction", () => {
  it("fetches pages with created_asc sort, max page size, display fields (no token)", async () => {
    const api = makeApi(8412);
    const r = await resolveSeededSet(api.transport, state(`?${W}`));
  ...
  it("groups draws by page: one count request plus one request per distinct page", async () => {
    const api = makeApi(8412);
    const s = state(`?${W}`);
    await resolveSeededSet(api.transport, s);
    const pages = new Set(
...
describe("resolveSeededSet determinism", () => {
  it("same state resolves to the same sounds in the same order, every time", async () => {
    ...
    const a = await resolveSeededSet(makeApi(8412).transport, s);
    const b = await resolveSeededSet(makeApi(8412).transport, s);
  ...
  it("returns sounds in draw order, not index order", async () => {
    const s = state(`?${W}`);
    const r = await resolveSeededSet(makeApi(8412).transport, s);
  ...
  it("quantization absorbs mid-week count drift", async () => {
    const s = state(`?${W}`);
    const monday = await resolveSeededSet(makeApi(8412).transport, s);
    const wednesday = await resolveSeededSet(makeApi(8431).transport, s);
...
describe("resolveSeededSet edge cases and errors", () => {
  it("returns fewer sounds when fewer exist than requested", async () => {
    const r = await resolveSeededSet(makeApi(3).transport, state(`?${W}`));
    if (!r.ok) throw new Error("expected ok");
  ...
  it("propagates a 401 from the count request as invalid-key", async () => {
    const r = await resolveSeededSet(unauthorized, state(`?${W}`));
  ...
  it("surfaces a page-level 429 as rate-limited, never a silent partial set", async () => {
    ...
    const r = await resolveSeededSet(transport, state(`?${W}`));
    expect(r).toEqual({
      ok: false,
...
describe("resolveLockedSet", () => {
  it("makes exactly one ID-filtered request: no count, no seed, no sort, no token", async () => {
    const api = makeLockedApi([301, 302, 303]);
    const r = await resolveLockedSet(api.transport, [302, 301, 303]);
    expect(r.ok).toBe(true);
  ...
  it("reorders the response to the requested (draw) order", async () => {
    const api = makeLockedApi([301, 302, 303]);
    const r = await resolveLockedSet(api.transport, [303, 301, 302]);
    if (!r.ok) throw new Error("expected ok");
  ...
  it("renders a missing id as an explicit placeholder while the rest load", async () => {
    ...
    const r = await resolveLockedSet(api.transport, [303, 302, 301]);
    if (!r.ok) throw new Error("expected ok");
  ...
  it("propagates a 401 as invalid-key", async () => {
    const r = await resolveLockedSet(unauthorized, [301]);
    expect(r).toEqual({ ok: false, error: { kind: "invalid-key" } });
  });
});
