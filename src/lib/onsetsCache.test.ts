...
describe("createOnsetsCache", () => {
  it("fetches once, caches onset times, and reports them", async () => {
    ...
    const c = createOnsetsCache(transport, () => "TOK");
    ...
    expect(vi.mocked(transport).mock.calls[0][0]).toBe(
      "https://freesound.org/apiv2/sounds/7/analysis/?token=TOK",
    );
  });
  ...
  it("treats malformed or empty onset payloads as unavailable", async () => {
    const c = createOnsetsCache(respond(200, { onset_times: "nope" }), () => "TOK");
    c.ensureOnsets(1);
    const c2 = createOnsetsCache(respond(200, { onset_times: [] }), () => "TOK");
    c2.ensureOnsets(2);
    await settle();
  ...
  it("does nothing without a token", () => {
    const transport = respond(200, { onset_times: [1] });
